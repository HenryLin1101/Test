...
Date::Date(int year, int month, int day) : year(year), month(month), day(day) 
{
    ...
    //but if year is a const int, can't use this 
}
...
protected:
    int id;
    Date birthday;
    //there should be a defalut constructor of Date()
    string name;
public:
    // constructor: specify the ID, birthday, and name of the animal
    Animal(int id, const Date& b, const string& n);
    //call by reference make it save time (don't need to copy)
...
{  // direct assignment of types int, Datem and string is fine
    // Date can be assign like this, because there is no pointer, and won't happen shallow copy
}

Animal::~Animal()
{
}
//const mean it won't change member variable
int Animal::getID() const
{
    //this is a poniter who point id
    return this->id;   
}
